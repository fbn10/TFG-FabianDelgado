"""Cargador del programa XDP de ORION (NAT + filtrado).

Funciones:
  1. Compila y atacha el programa XDP a las interfaces del proyecto.
  2. Lee de Elasticsearch los mapeos NAT (orion-nat-mappings) y las reglas
     de filtrado (orion-rules), y rellena los mapas eBPF con ellos.
  3. Refresca las reglas cada N segundos para que los cambios desde la
     API se propaguen sin reiniciar el loader.

Uso (necesita sudo):
    sudo -E python3 tools/xdp_nat.py
"""
...
# Compilar y atachar
print("\nCompilando programa XDP...")
b = BPF(src_file="ebpf/orion_xdp.c")
...
print(f"\nXDP activo. Refrescando reglas cada {RULES_REFRESH_SEC}s. Ctrl+C para parar.\n")
