...
// Observabilidad: ring buffer kernel -> user-space (Fase 9). Cuando XDP
// devuelve XDP_REDIRECT bypassa el stack del kernel y Packetbeat (AF_PACKET)
// deja de ver el trafico. Para recuperar visibilidad emitimos un evento
// ligero (~32 bytes) por cada paquete redirigido. Un proceso Python
// (tools/flow_consumer.py) consume el ring y lo indexa en orion-flows. Es
// el patron de Cilium Hubble / Tetragon / Falco.
struct flow_event {
    __u64  ts_ns;        // timestamp del kernel
    __be32 saddr_orig;   // src antes del NAT
    __be32 daddr_orig;   // dst antes del NAT
    __be32 saddr_post;   // src despues del NAT (== saddr_orig si no se reescribio)
    __be32 daddr_post;   // dst despues del NAT
    __be16 sport;        // L4 puerto origen (0 si no es TCP/UDP)
    __be16 dport;        // L4 puerto destino
    __u8   proto;        // ip->protocol
    __u8   iface_idx;    // 0=enp6s0f0, 1=eno1, 2=enp6s0f1
    __u8   action;       // 2=REDIRECT (unico que emitimos; futuro: 0=PASS, 1=DROP)
    __u8   _pad;         // padding para alinear a 32 bytes
    __u32  pkt_len;      // longitud del paquete
};

// Ring buffer de 1 MB (256 paginas de 4KB).
// Si se llena, ringbuf_reserve() devuelve NULL: el evento se pierde
// silenciosamente, pero el paquete sigue procesandose normalmente.
// Asi protegemos el data path: nunca bloqueamos por culpa de la observabilidad.
BPF_RINGBUF_OUTPUT(flow_events, 256);

// Contador de eventos perdidos por overflow del ring (cuando consumer es lento).
// Visible desde user-space para alertar en logs o en la GUI.
// PERCPU para no usar locks.
BPF_PERCPU_ARRAY(ringbuf_lost, u64, 1);


// Helper para emitir un flow_event. Si el ring esta lleno, incrementa
// ringbuf_lost y vuelve sin emitir nada.
static __always_inline void emit_flow_event(
    __be32 saddr_orig, __be32 daddr_orig,
    __be32 saddr_post, __be32 daddr_post,
    __be16 sport, __be16 dport,
    __u8 proto, __u8 iface_idx, __u8 action,
    __u32 pkt_len)
{
    struct flow_event *ev = flow_events.ringbuf_reserve(sizeof(*ev));
    if (!ev) {
        // Ring lleno -> el consumer no esta vaciando lo suficientemente rapido.
        // Incrementamos el contador de perdidas y seguimos sin bloquear.
        u32 k = 0;
        u64 *lost = ringbuf_lost.lookup(&k);
        if (lost) *lost += 1;
        return;
    }
    ev->ts_ns      = bpf_ktime_get_ns();
    ev->saddr_orig = saddr_orig;
    ev->daddr_orig = daddr_orig;
    ev->saddr_post = saddr_post;
    ev->daddr_post = daddr_post;
    ev->sport      = sport;
    ev->dport      = dport;
    ev->proto      = proto;
    ev->iface_idx  = iface_idx;
    ev->action     = action;
    ev->_pad       = 0;
    ev->pkt_len    = pkt_len;
    flow_events.ringbuf_submit(ev, 0);
}
...
after_l4:;
    ...
    if (my_iface == 2) {
        ...
        if (new_daddr) {
            ...
            if (nat_recompute_checksums(ctx, ip, old, new) < 0)
                return XDP_DROP;
            emit_flow_event(ip->saddr, old, ip->saddr, new,
                            sport, dport, ip->protocol, my_iface, 2, pkt_len);
        ...
        if ((ip->daddr & bpf_htonl(0xFFFFFF00)) == bpf_htonl(0x0A000200)) {
            emit_flow_event(ip->saddr, ip->daddr, ip->saddr, ip->daddr,
                            sport, dport, ip->protocol, my_iface, 2, pkt_len);
            return bpf_redirect(*idx_internal_redir, 0);
        }
    ...
    // XDP_REDIRECT: emitir el paquete directamente por la NIC destino sin
    // pasar por el stack del kernel. Es la version "alto rendimiento" del NAT.
    //
    // bpf_fib_lookup consulta la FIB del kernel desde XDP. Dadas (saddr,
    // daddr) ya reescritas devuelve el ifindex de salida y las MACs (smac
    // propia + dmac del nexthop). Si la cache ARP no tiene aun el nexthop,
    // devuelve distinto de SUCCESS y caemos a XDP_PASS para que el stack lo
    // gestione (la siguiente vez ya estara en cache).
    //
    // macvlan_up.sh quita la ruta por defecto de 'main' y la mete en la
    // tabla 100 con un 'ip rule from <EXT_IP> lookup 100'. bpf_fib_lookup
    // con flags=0 SOLO mira 'main' y NO respeta los 'ip rule', asi que el
    // saliente (cliente -> Internet) no encontraba ruta y caia a XDP_PASS:
    // por eso el modo REDIRECT no funcionaba para el NAT hasta indicarle la
    // tabla con BPF_FIB_LOOKUP_TBID:
    //   - Saliente (my_iface==0): default route -> tabla 100.
    //   - Entrante (my_iface==1): destino 10.0.2.0/24 (cliente), ruta conexa
    //     que sigue en 'main' -> consulta normal con flags=0.
    #ifndef BPF_FIB_LOOKUP_TBID
    #define BPF_FIB_LOOKUP_TBID (1U << 3)
    #endif
    #define ORION_RT_TABLE 100
    ...
    long rc;
    if (my_iface == 0) {
        // Saliente: la ruta por defecto vive en la tabla 100.
        // El campo 'tbid' (u32) ocupa el mismo offset que la union de
        // campos VLAN (h_vlan_proto/h_vlan_TCI). Las cabeceras con las que
        // BCC compila aun no nombran 'tbid', pero el kernel 6.17 en runtime
        // si lo interpreta. Escribimos los 4 bytes en ese offset via cast.
        *((__u32 *)&fib.h_vlan_proto) = ORION_RT_TABLE;
        rc = bpf_fib_lookup(ctx, &fib, sizeof(fib), BPF_FIB_LOOKUP_TBID);
    } else {
        // Entrante: la ruta al cliente (10.0.2.0/24) esta en 'main'.
        rc = bpf_fib_lookup(ctx, &fib, sizeof(fib), 0);
    }
    if (rc != BPF_FIB_LKUP_RET_SUCCESS)
        return XDP_PASS;   // fallback: el stack se encarga

    // Re-validar bounds antes de tocar el frame Ethernet
    if ((void *)(eth + 1) > data_end)
        return XDP_PASS;

    // Reescribir MACs del frame con las que devolvio bpf_fib_lookup.
    // smac = MAC origen (la propia NIC de salida), dmac = MAC nexthop.
    __builtin_memcpy(eth->h_source, fib.smac, 6);
    __builtin_memcpy(eth->h_dest,   fib.dmac, 6);

    // Emitir evento al ring buffer ANTES del redirect (visibilidad en user-space).
    // saddr/daddr "post" son los que ya tiene ip->saddr/daddr tras la reescritura;
    // saddr/daddr "orig" son los pre-NAT (segun direccion del flujo).
    if (my_iface == 0) {
        // Saliente: se reescribio saddr (cliente -> internet)
        emit_flow_event(old, ip->daddr, new, ip->daddr,
                        sport, dport, ip->protocol, my_iface, 2, pkt_len);
    } else {
        // Entrante: se reescribio daddr (internet -> cliente)
        emit_flow_event(ip->saddr, old, ip->saddr, new,
                        sport, dport, ip->protocol, my_iface, 2, pkt_len);
    }

    // El destino del redirect NO puede ser una macvlan: el driver macvlan
    // no implementa ndo_xdp_xmit, asi que bpf_redirect hacia ella se
    // descarta silenciosamente. La ruta saliente de la tabla 100 sale por
    // eno1.cliA (macvlan), por lo que fib.ifindex apuntaria a esa macvlan.
    // Redirigimos a la NIC FISICA padre usando iface_indexes, conservando
    // las MAC que ya devolvio bpf_fib_lookup (origen = MAC de la macvlan,
    // destino = MAC del gateway). El frame sale fisicamente por eno1 con la
    // identidad de la macvlan, exactamente igual que haria el kernel.
    //   Saliente (my_iface==0) -> eno1      = iface_indexes[1]
    //   Entrante (my_iface==1) -> enp6s0f0  = iface_indexes[0]
    u32 phys_key = (my_iface == 0) ? 1 : 0;
    u32 *phys_ifindex = iface_indexes.lookup(&phys_key);
    if (!phys_ifindex)
        return XDP_PASS;
    return bpf_redirect(*phys_ifindex, 0);
}
